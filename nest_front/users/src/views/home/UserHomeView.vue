...
<template><main class="home-page"><section class="home-card"><p class="brand">智乡云</p><h1>您好，{{ authStore.profile?.name }}</h1><p>农业服务已就绪，可查看种植知识并提交田间智能分析。</p><p v-if="errorMessage" class="error" role="alert">{{ errorMessage }}</p><div class="actions"><van-button round type="primary" @click="router.push({ name: 'user-agriculture' })">进入农业服务</van-button><van-button round plain type="primary" @click="router.push({ name: 'user-health' })">进入健康服务</van-button><van-button round plain type="primary" @click="logout">退出登录</van-button></div></section></main></template>
