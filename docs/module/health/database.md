...
当前项目的 `users.id` 为 `INTEGER`。健康模块五张核心表的自身主键均使用
`INTEGER GENERATED BY DEFAULT AS IDENTITY`，所有 `user_id` 同样使用
`INTEGER` 并通过外键关联 `users.id`。

这样避免在当前单库模型中混用 UUID 与 INTEGER 主键；健康模块内的
`template_id` 也使用 `INTEGER` 关联 `health_metric_templates.id`。
...
|---|---|---:|---|
| id | INTEGER | 否 | 主键，自增 |
| user_id | INTEGER | 否 | 用户 ID，关联 `users.id` |
| sex | VARCHAR(20) | 是 | 性别 |
...
|---|---|---:|---|
| id | INTEGER | 否 | 主键，自增 |
| user_id | INTEGER | 否 | 创建用户，关联 `users.id` |
